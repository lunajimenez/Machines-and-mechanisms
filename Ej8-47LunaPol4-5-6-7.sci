// -----------------------------------------------------------------
// Code name:         Cam Design with Polynomial 4-5-6-7 Function. 
...
// Función Polinomio 4-5-6-7
function [s,v,a,j]=Func4567(theta,Beta,h,tipo)
    if tipo == "subida" then
        c0 = 0; c1 = 0; c2 = 0; c3 = 0;
    ...
    if PdM(m)=="D" then
        ...
        for teta=0:BETA(m)/50:BETA(m)//Ciclo para calcular s,v,a y j
            if PdM(m)=="R" then  // Subida
                [s(i),v(i),a(i),j(i)]=Func4567(teta,BETA(m),h,"subida")
            else                 // Bajada
                [s(i),v(i),a(i),j(i)]=Func4567(teta,BETA(m),h,"bajada")
            end
...
// Gráficas de s,v,a y j con función Polinomio 4-5-6-7
scf(1)
graficador(TETA,s,ang,["Gráfica de posición (Polinomio 4-5-6-7)" "" "s (mm)"],411)
graficador(TETA,v,ang,["Gráfica de velocidad (Polinomio 4-5-6-7)" "" "v (mm/s)"],412)
graficador(TETA,a,ang,["Gráfica de aceleración (Polinomio 4-5-6-7)" "" "a (mm/s^2)"],413)
graficador(TETA,j,ang,["Gráfica de jerk (Polinomio 4-5-6-7)" "" "j (mm/s^3)"],414)

// Mostrar valores máximos
printf("Función: Polinomio 4-5-6-7\n")
